...
import { optionalKey, useLocale, useRenderLink, useT } from "../i18n";
...
function TrialThreadLink({ threadId, clientRef }: { threadId: string | null; clientRef: string | null }) {
  ...
  // El enlace lo pinta quien monta el paquete (`renderLink`): en la consola es
  // `next/link`, para que el cajón siga abierto al ir a mirar el hilo; en la
  // aplicación de escritorio, una orden a la cáscara. Sin él, un `<a>`.
  return (
    <>
      {renderLink({
        href,
