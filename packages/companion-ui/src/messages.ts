...
export type CompanionMessageKey = keyof typeof companionMessages;

/** `{var}` → valor. Misma regla que `i18n/messages.ts` de la consola. */
export function formatMessage(
  locale: Locale,
  key: CompanionMessageKey,
  vars?: Record<string, string | number>,
): string {
  const entry = companionMessages[key];
