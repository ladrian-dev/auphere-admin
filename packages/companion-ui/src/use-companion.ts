...
import type { CompanionThread } from "./wire";

import { cacheRunIds, loadRunIds, rememberRunId } from "./storage";
import { type PageContext, type Transport, makeCompanionClient } from "./transport";
...
/**
 * `transport` es lo único que sabe de red (spec 003): la consola pasa el de
 * `fetch`, la aplicación de escritorio el de IPC. Tiene que ser estable
 * (creado una vez), porque los callbacks dependen de él.
 */
export function useCompanion(transport: Transport) {
  const companionClient = React.useMemo(() => makeCompanionClient(transport), [transport]);
