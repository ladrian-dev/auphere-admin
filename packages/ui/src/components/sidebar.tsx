...
// Linear / Vercel pattern: collapsed sidebar is 56px wide so icons can sit on a
// 32px square hit target with 12px gutters on each side. 48px (the shadcn
// default) feels cramped once the operator hovers a row.
...
function SidebarGroupAction({
  ...
  return useRender({
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          "absolute top-4 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
          className
        ),
      },
      props
...
function SidebarMenuItem({ className, ...props }: React.ComponentProps<"li">) {
  return (
    <li
      ...
      // In icon-only mode the <li> centres its single child so the 32px button
      // (or any custom trigger like UserMenu) sits in the middle of the 56px
      // rail instead of being anchored to the left edge by SidebarGroup p-2.
      className={cn(
...
const sidebarMenuButtonVariants = cva(
  ...
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md px-2 py-2 text-left text-sm ring-sidebar-ring outline-hidden group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:p-0! group-data-[collapsible=icon]:[&>span]:hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mx-4 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-3 py-1 group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
    />
