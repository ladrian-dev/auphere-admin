...
- **Cada tarea cita sus requisitos**: termina con una línea `_Requisitos: 1.2, 3.4_`.
  Una tarea sin trazabilidad no entra en el plan.
- **Cada tarea entregada se anota**: `Entregado: PR #NNN (rama), fusionado YYYY-MM-DD`.
- **Test primero (§VII)**: los bloques de tests NO son opcionales. Se escriben,
  se ven en rojo, y solo entonces se implementa.
- **Aislamiento (§I)**: si la spec declaró garantías tocadas, existe una tarea
  por garantía que añade su test en `tests/isolation/`. Bloquea el merge.
- **Licencias (§VIII)**: si el plan instala una dependencia nueva, hay una tarea
  que deja escrita la licencia y el párrafo que permite el uso.
- **Medidor**: si la feature gasta modelo, reloj de máquina o herramienta de pago,
  hay una tarea que la conecta al medidor que ve el partner.
...
### Tests for User Story 1 (OBLIGATORIO — constitución §VII: el test se escribe y se ve en rojo antes de la implementación) ⚠️
...
```bash
# Launch all tests for User Story 1 together (siempre):
