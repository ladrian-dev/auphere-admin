...
class SendInteractive(ToolBase):
    ...
    description = (
        ...
        "list, CTA URL, or catalog product cards) as the response to the "
        "user. Exactly ONE of ``buttons`` (1-3 reply buttons), ``list`` "
        "(1-10 selectable items), ``cta_url`` (single URL-opening "
        "button), or ``products`` (1-30 product_retailer_id from the "
        "store catalog — native product cards the customer can add to "
        "their WhatsApp cart) must be set. This tool is the TERMINAL "
        "action of the turn — after "
