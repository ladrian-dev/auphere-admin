...
| `src/components/result/ResultView.tsx` | Plazo de respuesta prometido ("te escribirán en los próximos días"). |
...
| `src/components/ui/PartnerSticker.tsx` | Fórmula exacta del sello "Partner oficial de Auphere" (¿partner oficial, partner tecnológico, embedded partner?). |

## Configuración y marca

- ~~Correo destino de leads~~ — decidido el 2026-09-17: `contacto+event@auphere.com`. Falta solo el remitente verificado en Resend (`LEADS_FROM`). Ver `docs/CONFIG.md`.
- ¿Amacrux quiere recibir también una copia directa? `LEADS_TO` admite varias direcciones separadas por comas.
