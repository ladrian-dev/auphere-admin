...
def scheduler_tasks(ctx: WorkerContext, *, heartbeat: bool = True) -> list[asyncio.Task[None]]:
    ...
    return ([_heartbeat_task(ctx)] if heartbeat else []) + [
        ...
        # ADR-035 — daily cobranza sweep at the tenant's LOCAL hour. Needs
        # redis only for the "already swept today" marker; correctness
        # comes from the per-(account, stage, due) idempotency in Postgres.
        _spawn(
            "cobranza-reminder-cron",
