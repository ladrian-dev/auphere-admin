...
async def test_resolves_via_postgres_when_cache_empty(db_session, fake_redis, seed_tenants):
    ...
    resolved = await resolve_tenant(db_session, fake_redis, "ycloud", "phone-1")
    assert resolved == tid
...
async def test_resolves_via_cache_on_second_call(db_session, fake_redis, seed_tenants, monkeypatch):
    ...
    await resolve_tenant(db_session, fake_redis, "ycloud", "phone-2")
    ...
    resolved = await resolve_tenant(db_session, fake_redis, "ycloud", "phone-2")
    assert resolved == tid


async def test_unknown_identifier_raises(db_session, fake_redis):
    with pytest.raises(TenantNotFound):
        await resolve_tenant(db_session, fake_redis, "ycloud", "ghost-xyz")
...
async def test_paused_channel_does_not_resolve(db_session, fake_redis, seed_tenants):
    ...
    with pytest.raises(TenantNotFound):
        await resolve_tenant(db_session, fake_redis, "ycloud", "paused-1")
...
async def test_invalidate_cache(db_session, fake_redis, seed_tenants):
    ...
    await resolve_tenant(db_session, fake_redis, "ycloud", "phone-3")
    assert await fake_redis.get("nexus:tenant_resolve:ycloud:phone-3") is not None
    await invalidate_tenant_cache(fake_redis, "ycloud", "phone-3")
    assert await fake_redis.get("nexus:tenant_resolve:ycloud:phone-3") is None
...
async def test_corrupt_cache_falls_back_to_db(db_session, fake_redis, seed_tenants):
    ...
    await fake_redis.setex("nexus:tenant_resolve:ycloud:phone-4", 60, "not-a-uuid")
    resolved = await resolve_tenant(db_session, fake_redis, "ycloud", "phone-4")
    assert resolved == tid
...
async def test_resolve_returns_uuid_type(db_session, fake_redis, seed_tenants):
    ...
    resolved = await resolve_tenant(db_session, fake_redis, "ycloud", "phone-5")
