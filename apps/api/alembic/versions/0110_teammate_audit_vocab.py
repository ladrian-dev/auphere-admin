...
ROWS = (
    (
        ...
        "teammates",
        "info",
        "{actor} creó el teammate {teammate}.",
        "{actor} created the teammate {teammate}.",
    ),
    (
        "teammate.updated",
        "teammates",
        "info",
        "{actor} cambió el teammate {teammate}.",
        "{actor} changed the teammate {teammate}.",
    ),
    (
        "teammate.archived",
        "teammates",
        "info",
        "{actor} archivó el teammate {teammate}.",
        "{actor} archived the teammate {teammate}.",
    ),
    (
        "local_policy.ceiling_changed",
        "teammates",
        "warning",
        "{actor} puso el techo de ejecución local en {ceiling}.",
        "{actor} set the local execution ceiling to {ceiling}.",
    ),
    (
        "local_policy.pref_changed",
        "teammates",
        "info",
        "{actor} cambió su preferencia de ejecución local ({executable}: {mode}).",
        "{actor} changed their local execution preference ({executable}: {mode}).",
    ),
    (
        "local_exec.allowed_once",
        "teammates",
        "info",
        "{actor} permitió una vez {executable} en {machine}.",
        "{actor} allowed {executable} once on {machine}.",
    ),
    (
        "local_exec.allowed_by_policy",
        "teammates",
        "info",
        "{actor} tenía «permitir siempre» y {executable} se ejecutó en {machine}.",
        "{actor} had “always allow” and {executable} ran on {machine}.",
    ),
    (
        "local_exec.denied_by_policy",
        "teammates",
        "warning",
        "{actor} tenía «nunca» y {executable} no se ejecutó.",
        "{actor} had “never” and {executable} did not run.",
    ),
)
