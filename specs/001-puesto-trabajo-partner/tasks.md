...
- [ ] T057 Firma y notarización del paquete. _Requisitos: 9.1, 9.2_
      ...
      **La notarización también quedó resuelta el 2026-09-13**, el mismo día: API key `NNFTSY329V` con acceso **Developer** (el mínimo suficiente), verificada con un round-trip autenticado real contra Apple (`xcrun notarytool history` → `rc=0`). No se reutilizó ninguna de las dos que ya existían (`Api revenue cat`, `[Expo] EAS Submit`): son de producción de Facelad y su `.p8` no se puede volver a descargar.
      **Lo que sigue pendiente, y ya no es esta tarea:** construir la cadena que use todo eso. **Pasa a la spec 008** (`specs/008-empaquetado-firma-y-canal/`), donde el certificado y la clave son secretos de repositorio desde el 2026-09-14 y la firma corre en integración continua, no en el llavero de nadie.
      **Windows** sigue fuera, y no por los certificados: depende de que la contención esté portada (T039/T043). Firmar allí prometería lo que no se cumple.
