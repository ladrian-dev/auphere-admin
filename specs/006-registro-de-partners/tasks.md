...
- [~] T010 Desplegar la Fase 2 sola a staging y observar los límites del login antes de seguir. Producción tiene 3 clientes con tráfico: un limitador que de golpe empieza a contar de verdad se mira antes de apilarle un formulario público encima. _Requisitos: 7.1_
      **PARCIAL, y digo exactamente qué falta.** Desplegado y observado en staging el 2026-09-14.
      **La mitad que importaba para producción está cubierta**: 11 intentos de login desde una misma IP con 11 correos distintos → **los 11 `401`, ninguno `429`** (contados en los accesos de `/nexus/staging/api`: `11 "POST /console/auth/login HTTP/1.1" 401 Unauthorized`). A ritmo normal el limitador **no echa a nadie**, que era el riesgo para los 3 clientes con tráfico. Y las sesiones nuevas se crean sin error: el alta completa de punta a punta terminó dentro de la consola.
      **Lo que NO pude comprobar es la denegación en ráfaga, y no porque esté rota: mi prueba no podía.** `core/rate_limit.allow()` es un token bucket donde `per_minute` es también la capacidad, así que a 3/min se repone una ficha cada 20 s. Cuatro altas desde la misma IP en ~30 s dieron los cuatro `202` — consistente con la reposición, no con un cubo que no cuenta. Un formulario de navegador no produce una ráfaga: entre clic y clic pasa más tiempo del que tarda el cubo en recuperarse.
      **Para cerrarlo** hace falta disparar N+1 peticiones en menos de lo que tarda una ficha, lo que pide un script contra la consola y no un navegador.
...
- [~] T053 Ejecutar [quickstart.md](quickstart.md) entero, incluida la comparación de **tiempos** de respuesta entre correo existente y nuevo — un canal lateral temporal deja el `CE-004` abierto aunque el cuerpo sea idéntico. _Requisitos: CE-001 … CE-006_
      **EJECUTADO EN STAGING el 2026-09-14, salvo la comparación de tiempos.**
      **CE-001 en verde, de punta a punta y desde un navegador sin sesión**: `/signup` → formulario → correo **Delivered** en Resend → enlace → nombrar empresa → **dentro de la consola como Propietario**, partner «Agencia de Prueba 006», **«Estás en el plan gratuito»** con *Agentes 0 · Personas 1*. Nadie ejecutó nada. El `tier` es Free por ausencia de fila, que es lo que el quickstart exige.
      **CE-004, la mitad del cuerpo**: la respuesta es **idéntica** para una dirección con cuenta (`contacto@ladrian.dev`) y otra sin ella (`contacto+alta1@ladrian.dev`) — mismo texto, mismo condicional «si esa dirección se puede registrar».
      **CE-006**: el botón de Google se pinta porque Google está configurado, y el alta con contraseña funciona sin tocarlo.
      **Lo que falta**: la comparación de **tiempos** de respuesta. Pide 20 peticiones de cada y comparar medianas, y eso no se hace con un formulario de navegador — el ruido del navegador y de la red tapa el canal lateral que se quiere medir. Necesita un script.
      **Hallazgo secundario**: el paso de Mailhog del quickstart es documentación muerta. `services/email.py` sólo habla con Resend, así que en local `email_enabled` es `False` y no se manda nada a ninguna parte; el enlace en desarrollo se saca de la base.
- [X] T054 Verificar entregabilidad real del correo a los tres dominios más frecuentes del ICP. Si cae en spam, es un hallazgo con su corrección, no una suposición que se arrastra. _Requisitos: 1.1, research R8_
      **HECHO, y encontró dos cosas — que es para lo que existía.**
      **Primero, que no llegaba nada.** El dominio no estaba verificado en Resend: `403 The auphere.com domain is not verified`, y la API respondía `202 «revisa tu correo»` igual. Callejón sin salida silencioso para todo el que usara el alta con contraseña. Se verificó el dominio y se arregló el lado del código (**T059**).
      **Segundo, de quién venía.** Salía de `facturacion@auphere.com` por herencia de `receipt_from_email`. Corregido a `no-reply@` (**T060**).
      **Evidencia de entrega**: `{"event": "email.sent", "to": ["contacto+alta2@ladrian.dev"]}` en `/nexus/staging/api`, y **Delivered** en el panel de Resend, con el enlace del alta dentro y funcionando.
      **Queda por probar**: los otros dos dominios del ICP. Con el dominio del remitente verificado y SPF/DKIM en regla, el riesgo baja mucho, pero no es lo mismo que haberlo visto.
...
- [X] T058 La lista pública del proxy de la consola: `/signup` y `/auth/google/callback` tienen que poder abrirse **sin sesión**. _Requisitos: 1.1, 5.6_
      **HECHO, y es el fallo más serio de la spec.** El alta se desplegó a staging con la verificación entera en verde y **sin poder usarse**: `proxy.ts` rebota a `/login` todo lo que no esté en `PUBLIC`, y no estaban ni `/signup` ni la vuelta de Google. Con las banderas encendidas, US1 y R5 habrían estado muertas las dos, en silencio.
...
- [X] T061 El idioma del correo es el de quien lo lee. _Requisitos: 1.1, 5.6_
      **HECHO.** Visto en staging tras arreglar el remitente: la página decía «Create your account.» y el asunto llegaba como «Crea tu cuenta de Auphere». `signup-form.tsx` mandaba `locale: "es"` literal a la acción de servidor.
      Es el **mismo fallo que `getT("es")`**, un escalón más abajo: una mitad del producto decidiendo el idioma por su cuenta cuando el `LocaleProvider` ya lo había resuelto bien. Por eso el guardián vive en el mismo fichero y ahora cubre las dos formas. Rojo primero: 1 de 20, y señalaba el fichero exacto. Mutación: volver al literal → 1 rojo.
