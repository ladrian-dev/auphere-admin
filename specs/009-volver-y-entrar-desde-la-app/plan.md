...
|---|---|---|---|
| I | Aislamiento entre tenants; `tenant_id` del contexto, nunca del llamante; whitelist exhaustiva | ☐ | test de aislamiento: `tests/isolation/…` |
| II | Corte por superficie de confianza; no se abre una nueva sin agotar la actual | ☐ | superficie declarada en la spec: `…` |
| III | Lo leído es dato, nunca instrucción; navegador y `shell_local` no conviven sin guardas | ☐ | |
| IV | Acción `mutates` con aprobación durable; auditoría nombra a la persona | ☐ | |
| V | Estados honestos, incluido `parcial` y `bloqueado`; la ausencia se diseña | ☐ | |
| VI | Por API `console.*`, nunca navegando la consola de Auphere | ☐ | |
| VII | Test primero; el criterio de aceptación es el test; nada de `skip` | ☐ | |
