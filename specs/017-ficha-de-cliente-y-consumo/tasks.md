...
- [X] T027 [US1] `apps/console/e2e/record.spec.ts`: recorrido de la ficha con owner, builder y analyst (pasos, botón, grupos, «Más», barra de borrador); añadir `/clients/{ref}/capabilities` (aunque redirija) a `a11y.spec.ts`; 360 y 1 920 px, ES/EN. _Requisitos: 2.3, 12.3_ Entregado: rama 017, 2026-09-26. `e2e/record.spec.ts` con 6 casos (cabecera y botón único, tres grupos con once pestañas, «Más» sin eliminar antes de archivar, borrador visible y revisable desde Conversaciones, y la URL de Capacidades). 30 pasan y 2 se saltan **en voz alta**: builder y analyst necesitan `E2E_BUILDER_*`/`E2E_ANALYST_*`, que el arnés no siembra; su reparto por rol está clavado en los módulos puros. `/capabilities` no entra en la barrida de axe —auditar una redirección destruye el contexto a mitad y no dice nada— sino como prueba de que la URL existe y aterriza en `/tools`.
- [ ] T028 [US1] Cerrar `parity.md` §Iteración 1 (las 23 filas revisadas contra el código) y `evidence/iteracion-1.md` (capturas por rol, resultados de suites); log de sesión `kb/…/sessions/2026-MM-DD-spec-017-iteracion-1.md`; merge a `develop`, despliegue y recorrido en staging. _Requisitos: 12.1, 12.2, 12.5_ **Parcial** (rama 017, 2026-09-26): `parity.md` §Iteración 1 revisada fila a fila contra el código (16 cerradas, 2 parciales, 5 pendientes, 2 diferidas a la it. 2), `evidence/iteracion-1.md` con el recorrido y las suites, y log de sesión `kb/…/sessions/2026-09-26-spec-017-iteracion-1.md`. **Pendiente**: decisión del owner sobre las 5 filas pendientes, y con ella el merge a `develop`, el despliegue y el recorrido en staging.
...
- [X] T029 [US3] Prototipo `packages/ui/src/stories/prototypes/capabilities.stories.tsx`: Integraciones arriba, grupos por función, tarjeta con conmutador/estado/modo/«Necesita X», «Ver todas», sin sector, buscador, lote; aprobación en `evidence/iteracion-2.md`. _Requisitos: 12.4_ **En revisión** (rama develop, 2026-09-26): 9 historias (con sector · ver todas · sin sector · todo conectado · buscando · sin resultados · solo lectura · catálogo vacío · móvil). Decisiones del owner incorporadas: avisar y dejar encender, «Nunca» en vez de «Bloqueada», y el bloque de integraciones desbloquea sin duplicar la pestaña. Falta la **aprobación del owner**. **Aprobado por el owner el 2026-09-26.**
- [X] T030 [P] [US3] Test primero `apps/api/tests/unit/test_capability_names.py`: el mapa `capability_names.py` cubre **todas** las herramientas del catálogo sembrado y todas las habilidades del bundle (nombre de negocio ES/EN, función, sectores); una entrada sin traducción falla el test; `function` ∈ {appointments, orders, messages, escalation, knowledge, other}. _Requisitos: 5.1, 5.6_ Entregado: rama develop, 2026-09-26 (9 casos). La cobertura de herramientas va en `tests/integration/test_capability_names_cover_catalog.py` y no en el unitario: el catálogo no es un fichero —lo siembran las migraciones y `seed_connectors.py`—, así que la única fuente que no se desfasa es la base. El test cazó 12 herramientas de WooCommerce que faltaban en el mapa.
- [X] T031 [US3] Crear `apps/api/src/nexus_api/api/console/capability_names.py` con el mapa y las funciones `business_name(key, kind, lang)`, `function_of`, `sectors_of` (a partir de `capability_tags` que nombren un vertical). _Requisitos: 5.1, 5.2_ Entregado: rama develop, 2026-09-26 (60 capacidades con nombre y descripción en ES y EN; `business_name`, `description_of`, `function_of`, `sectors_of`). Una capacidad que el catálogo añada antes que el mapa se enseña con su nombre técnico en «Otras»: esconderla sería peor.
- [X] T032 [P] [US3] Test primero `apps/api/tests/integration/test_console_capabilities.py`: `GET …/capabilities` agrupa por función; con sector filtra y devuelve `hidden_by_sector`; `all=true` incluye el resto marcado; `q=` busca en nombre y descripción del idioma; `recommended` = herramientas de la plantilla del sector; `usable=false` con conector no conectado; `mode.options` sin `needs_approval`; `PUT` con un cambio crea borrador (`draft_created`), escribe la lista blanca **solo** de ese tenant, audita `console.capability.update`; `PUT` `needs_approval` → 422; activar con conector ausente → 409 `connector_required`; `PUT …/tools` con `needs_approval` → 422. _Requisitos: 4.3, 5.1, 5.2, 5.3, 5.4, 5.5, 5.7, 5.8, 5.9_ Entregado: rama develop, 2026-09-26 (14 casos). **Cambio sobre lo que pedía la tarea**: encender una capacidad sin su integración NO es 409 `connector_required` sino 200 con `usable: false`, por decisión del owner del 2026-09-26 (avisar, no bloquear). Sí es 422 `mode_not_supported` pedir «requiere aprobación», en el endpoint nuevo y en el viejo. Los tests que necesitan una herramienta con conector **se la crean**: el catálogo sembrado cambia según el entorno y depender de él hacía pasar o fallar por razones ajenas a lo que se prueba.
- [X] T033 [US3] Implementar `apps/api/src/nexus_api/api/console/capabilities_client.py` (router bajo `/clients/{ref}/capabilities`, `client_scope("agents:read"/"agents:write")`, `ensure_draft`, reutilizando `tools.py`/`skills.py`), `schemas_capabilities_client.py`, registro del router, y el 422 en `tools.py`. _Requisitos: 5.1–5.9_ Entregado: rama develop, 2026-09-26 (`capabilities_client.py`, `schemas_capabilities_client.py`, router registrado, 422 en `tools.py`). Un cambio por llamada: la pantalla vieja mandaba la lista blanca entera y dos personas editando a la vez se pisaban. Un test antiguo que afirmaba que «requiere aprobación» se guardaba pasa a afirmar la regla nueva.
