...
- [ ] T021 [US1] Implementar `apps/console/src/components/clients/client-header.tsx` (+ `setup-steps.tsx`, `quota-meter.tsx`, `more-menu.tsx` sobre `DropdownMenu` y `ClientLifecycleActions` existente) y usarlo en `apps/console/src/app/(console)/clients/[ref]/layout.tsx`; la referencia y la zona horaria salen de la cabecera. _Requisitos: 1.1, 1.2, 1.3, 1.4, 1.5, 1.6_ **Parcial** (rama 017, 2026-09-25): hechos los pasos con su estado, el botón único del pendiente con su porqué, la frase de quién puede cuando el rol no, y el crédito como `Meter` o «Sin crédito asignado», montados en el layout de la ficha. **Pendiente**: el menú «Más» en la cabecera (hoy las acciones de ciclo de vida siguen donde estaban) y la verificación en navegador (el stack local estaba apagado).
...
- [ ] T025 [US2] Implementar `draft-bar-client.tsx` y `draft-diff-sheet.tsx` en `apps/console/src/components/clients/`, la acción `draftDiffAction` y `publishFromBarAction` en `apps/console/src/app/(console)/clients/[ref]/agent/actions.ts` (con `can()` y test en el arnés `__tests__/actions.test.ts`), y montarla en el layout de la ficha; los toasts de Ajustes/Herramientas/Habilidades/Conocimiento pasan a solo confirmar. _Requisitos: 3.1, 3.3, 3.4, 3.5_ **Parcial** (rama 017, 2026-09-25): `draft-bar-client.tsx` y `draft-diff-sheet.tsx` montados en el layout de la ficha, con `draftDiffAction` y `publishFromBarAction` (ambas con `can()` y test). El diff se pide al abrir la hoja, no al montar. **Pendiente**: la ventana de deshacer de 10 minutos (R3) y que los toasts de Ajustes/Herramientas/Habilidades/Conocimiento pasen a solo confirmar.
