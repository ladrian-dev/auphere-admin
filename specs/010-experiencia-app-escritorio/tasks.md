...
- [X] T043 [P] [US1] ~~Modo embebido en `apps/console/src/app/(console)/layout.tsx`~~ **Revertida el 2026-09-18 (E04).** Se construyó, se miró funcionando y duplicaba navegación, búsqueda, avisos e identidad; ver [`contracts/console-en-la-ventana.md`](./contracts/console-en-la-ventana.md). `apps/console` vuelve a no saber que la aplicación existe y `shell-detect.test.ts` recupera su afirmación original: `@/lib/shell` se importa **exactamente una vez** — _Requisitos: 1.3_
...
- [X] T141 **`./scripts/verify.sh` entero en verde, las dos mitades.** JS: consola, panel, `@nexus/ui`, `@nexus/companion-ui` y escritorio —927 pruebas de escritorio, 160 del paquete compartido, 284 de la consola, typecheck, lint y `next build`—. Python: ruff, ruff format y `mypy --strict` en api, worker, channels y mcp; **3577 pruebas de la API** (313 saltadas, 4 xfailed) y 419 del worker, cero fallos. Los cuatro gates de interfaz del workspace, cada uno con su prueba: **estados** (`screen-states-1/2`, `turn-states`), **accesibilidad** (`pnpm test:a11y` con axe sobre el binario construido, cero graves y críticas en los dos temas, más `keyboard`, `a11y-controls` y `motion-and-zoom`), **responsive** (`long-text` con cadenas alemanas) y **tokens** (`shell-tokens`, `product-guards` y el test de contraste de `@nexus/ui`)
